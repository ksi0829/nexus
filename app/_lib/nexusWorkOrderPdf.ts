...
export async function createWorkOrderPdf(input: WorkOrderPdfInput) {
  ...
  sheet.innerHTML = `
    ...
    <style>
      *{box-sizing:border-box}.header{display:grid;grid-template-columns:1fr 250px;border:1.5px solid #111}.title{height:118px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px;border-right:1px solid #111}.title strong{font-size:30px;letter-spacing:14px;border-bottom:2px solid #111;padding:0 14px 8px}.title time{font-size:11px}.issuer{display:grid;grid-template-rows:38px 1fr}.issuer b,.issuer strong{display:flex;align-items:center;justify-content:center;border-bottom:1px solid #111;font-size:12px}.issuer strong{border-bottom:0;font-size:18px}
